    ...
    <div
      ...
      className={`border p-6 transition-all duration-500 ease-out ${
        ...
        testimonial.isPlaceholder
          ? 'border-gray-200 dark:border-gray-700 border-dashed bg-gray-50 dark:bg-gray-800'
          : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900'
      }`}
    >
      <div className={`text-4xl mb-2 ${testimonial.isPlaceholder ? 'text-gray-300 dark:text-gray-600' : 'text-gray-400 dark:text-gray-500'}`}>
        &ldquo;
      </div>
      <p className={`font-pixel italic mb-4 min-h-[60px] leading-relaxed ${
        testimonial.isPlaceholder ? 'text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300'
      }`}>
        {testimonial.quote}
      </p>
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
        <p className={`font-pixel ${testimonial.isPlaceholder ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-gray-100'}`}>
          {testimonial.name}
        </p>
        <p className={`text-sm font-pixel ${testimonial.isPlaceholder ? 'text-gray-400 dark:text-gray-500' : 'text-gray-500 dark:text-gray-400'}`}>
          {testimonial.role}
        </p>
